...
export class MapsService {
    constructor(
        @InjectRepository(Map) private mapRepoitory: Repository<Map>,
        ) {}
    
    // find all
    async getAllMaps(seasons?: number) {
        if(seasons){return this.mapRepoitory.find({relations: {season: true}, where: {season: {id: seasons}}})}
        else {return this.mapRepoitory.find();}
    }

    // Find one match by ID
    async getMapByID(id: number) {
        const map = await this.mapRepoitory.findOne({where: {id: id}, relations: ["season"], loadRelationIds: true});
        if (map) {
            return map;
        }
